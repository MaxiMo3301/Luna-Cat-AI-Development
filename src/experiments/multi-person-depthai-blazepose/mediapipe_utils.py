...
def non_max_suppression(bodies, nms_thresh):
    ...
    boxes = [[int(x*1000) for x in b.pd_box] for b in bodies]
    scores = [b.pd_score for b in bodies]
...
def detections_to_rect(body, kp_pair=[0,1]):
    ...
    for b in body:   # for multi-person_detection
        x_center, y_center = b.pd_kps[kp_pair[0]]
        x_scale, y_scale = b.pd_kps[kp_pair[1]]
        # Bounding box size as double distance from center to scale point.
        box_size = sqrt((x_scale-x_center)**2 + (y_scale-y_center)**2) * 2
        b.rect_w = box_size
        b.rect_h = box_size
        b.rect_x_center = x_center
        b.rect_y_center = y_center

        rotation = target_angle - atan2(-(y_scale - y_center), x_scale - x_center)
        b.rotation = normalize_radians(rotation)
...
def rect_transformation(body, w, h, scale = 1.25):
    ...
    for b in body:  # for multi-person
        width = b.rect_w
        height = b.rect_h
        rotation = b.rotation
        if rotation == 0:
            b.rect_x_center_a = (b.rect_x_center + width * shift_x) * w
            b.rect_y_center_a = (b.rect_y_center + height * shift_y) * h
        else:
            ...
            b.rect_x_center_a = b.rect_x_center*w + x_shift
            b.rect_y_center_a = b.rect_y_center*h + y_shift

        # square_long: true
        long_side = max(width * w, height * h)
        b.rect_w_a = long_side * scale_x
        b.rect_h_a = long_side * scale_y
        b.rect_points = rotated_rect_to_points(b.rect_x_center_a, b.rect_y_center_a, b.rect_w_a, b.rect_h_a, b.rotation)
