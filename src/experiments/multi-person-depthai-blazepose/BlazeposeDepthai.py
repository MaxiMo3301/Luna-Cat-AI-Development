...
class BlazeposeDepthai:
    ...
    def __init__(self, input_src="rgb",
        ...
        self.bodies = None  # used in pose detection

        if self.multi_detection:
            print(
                "Warning: with multi-detection, smoothing filter is disabled "
                "and pose detection is forced on every frame.")
    ...
    def pd_postprocess(self, inference):
        ...
        # Decode bboxes
        self.bodies = mpu.decode_bboxes(self.pd_score_thresh, scores, bboxes, self.anchors,
                                        best_only=not self.multi_detection)

        # Non maximum suppression (not needed if best_only is True)
        if self.multi_detection:
            self.bodies = mpu.non_max_suppression(self.bodies, self.pd_nms_thresh)

        mpu.detections_to_rect(self.bodies)
        mpu.rect_transformation(self.bodies, self.frame_size, self.frame_size, self.rect_transf_scale)
    ...
    def next_frame(self):
        ...
        if self.force_detection or not self.use_previous_landmarks:
            ...
            # no pd render in original depthai :self.pd_render(annotated_frame)
            self.nb_pd_inferences += 1
        else:
            self.bodies = self.body_from_landmarks
            mpu.detections_to_rect(self.bodies)  # self.regions.pd_kps are initialized from landmarks on previous frame
            mpu.rect_transformation(self.bodies, self.frame_size, self.frame_size, self.rect_transf_scale)
        ...
        if self.force_detection:
            for b in self.bodies:
                frame_nn = mpu.warp_rect_img(b.rect_points, video_frame, self.lm_input_length, self.lm_input_length)
                ...
                self.lm_postprocess(b, inference)
                #self.lm_render(annotated_frame, b) # not render at here
                if b.lm_score >= self.lm_score_thresh:
                    if self.xyz:
                        self.query_body_xyz(b)
                    bodies.append(b)

        # elif: not force detection and this is only one body detected  implement later
        return video_frame, bodies
