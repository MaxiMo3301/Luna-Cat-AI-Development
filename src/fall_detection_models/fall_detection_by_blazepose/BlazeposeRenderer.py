...
# you can uncomment it for 3d visualization but it requires open3d
...
class BlazeposeRenderer:
    def __init__(self,
        ...
        if self.show_3d == "image":
            ...
            self.vis3d.create_grid([0, tracker.img_h, -z], [tracker.img_w, tracker.img_h, -z],
                                   [tracker.img_w, tracker.img_h, z], [0, tracker.img_h, z], 5, 2)  # Floor
            ...
            half_length = 3
            grid_depth = 5
            self.vis3d.create_grid([-half_length, 1, 0], [half_length, 1, 0], [half_length, 1, grid_depth],
                                   [-half_length, 1, grid_depth], 2 * half_length, grid_depth)  # Floor
            self.vis3d.create_grid([-half_length, 1, grid_depth], [half_length, 1, grid_depth],
                                   [half_length, -1, grid_depth], [-half_length, -1, grid_depth], 2 * half_length,
                                   2)  # Wall
        ...
        if output is None:
            ...
            # warning: tracker.video_fps is changed
            # tracker.video_fps = tracker.internal_fps
    ...
    def draw_landmarks(self, body):
        ...
        if self.show_landmarks:
            # print(len(body.landmarks[:,:3])) # 33 x 3
            # print(body.landmarks[:self.tracker.nb_kps,:3].shape)
            # print(body.norm_landmarks[:self.tracker.nb_kps,:3].shape)
            # print(body.landmarks[:,:3][23][2])
            # print(body.landmarks[:,:3][24][2])
            # body.landmarks[:self.tracker.nb_kps, :2] = body.norm_landmarks[:self.tracker.nb_kps, :2]
